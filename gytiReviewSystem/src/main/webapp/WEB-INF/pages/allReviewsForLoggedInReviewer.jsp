...
<div class=""
			style="padding-left: 0px; padding-right: 0px;"><jsp:include page="template/reviewHeader.jsp"></jsp:include>
</div>
...
<body style="background-color: #f5f5f5;">
	<div ng-controller="allReviewsByLoggedInReviewerController" ng-init="InitLoad()"
		ng-cloak="">
		...
		<div id="mainPanelBody" class="col-xs-10 col-md-10">
			...
			<div class="tabsdemoDynamicHeight">
				<md-content>
				 <md-tabs md-dynamic-height=""
					md-border-bottom="">
					<md-tab ng-click="showReviewdByLoggedInReviewer()">
					<!--  <md-tab ng-click="showReviewdByLoggedInReviewer()"> --> 
					 	<md-tab-label>
					 	<a>
									<md-tooltip md-direction="top">{{reviedByLoggedInReviewer.length}}&nbsp;
												projects to review</md-tooltip>Reviewed By You&nbsp;&nbsp; <strong><md-span
													class="notification-counter">{{reviedByLoggedInReviewer.length}}</md-span></strong>
						</a>
						</md-tab-label>
				...
				<div class="col-xs-2 col-md-2 pull-right">
					<div class="dropdown col-xs-4 col-xs-offset10" style="width: 150px; padding-left: 0px;">
						<button ng-click="ClickDownladPdf()" class="btn btn-success">Download</button>
					</div>
				</div>
				<div class="col-xs-12">&nbsp;</div>
				...
				<div>
					<table
						class="table table-responsive table-condensed table-hover table-striped" style="cellspacing:0; width:100%">
						<tr>
						...
						<tr ng-repeat="reviewedByLoggedinRev in pagedItems[currentPage] | orderBy:sort.sortingOrder:sort.reverse  | filter:SearchText">
							<td>{{reviewedByLoggedinRev.reviewedDate}}</td>
							<td>{{reviewedByLoggedinRev.participantId}}</td>
							<td>{{reviewedByLoggedinRev.projTitle}}</td>
							<td>{{reviewedByLoggedinRev.projCategory.join(",")}}</td>
							<td><p ng-repeat="assignedByAndReviewDoneByMapper in reviewedByLoggedinRev.assignedByAndReviewDoneByMapperSet">{{assignedByAndReviewDoneByMapper.assignedByName}}</p></td>
							<td><p ng-repeat="assignedByAndReviewDoneByMapper in reviewedByLoggedinRev.assignedByAndReviewDoneByMapperSet">
										<a style="color: pink" href="" data-toggle="modal"
											data-target="#ratingDetailsModal"
											ng-click="ShowDetailsReview(assignedByAndReviewDoneByMapper.ratingId,reviewedByLoggedinRev.projTitle,assignedByAndReviewDoneByMapper.reviewDoneByName)">{{assignedByAndReviewDoneByMapper.reviewDoneByName}}</a>
									</p></td>
							<td>{{reviewedByLoggedinRev.averageRating}}</td>
						</tr> 
					</table>
				</div>
				<div style="text-align: center;"
						ng-show="reviedByLoggedInReviewer.length==0">
						<p>
			...
			<div class="col-xs-12"
				...
				<div class="pagination pull-right" style="height: 50px;">
					<ul class="pagination" style="margin-top: -8px;">
						...
						<md-tab ng-click="showReviewdByOthers()"> 
						 	<md-tab-label  > 
						 	<a >
							 		<md-tooltip md-direction="top">{{reviedByOthersReviewer.length}}&nbsp;
															projects to review</md-tooltip>Reviewed by Others&nbsp;&nbsp;<strong
													class="notification-counter">{{reviedByOthersReviewer.length}}</strong>
							</a>
								</md-tab-label>
							<md-tab-body>
								<div>
				<!-- <div class=""><button ng-click="ClickDownladPdf()" class="btn btn-success">Download</button></div> -->
				<div class="" style="background-color: #23282e; height: 70px;">
				<div class="col-xs-12">&nbsp;</div>
				<!-- <div class="col-xs-3 col-md-3 pull-left" style="display: inline-block;">
					<div class="col-xs-3" style="padding-right: 0px; color: white">Show
				...
				<div>
					<table
						...
						<tr ng-repeat="reviewedByOtherRev in pagedItems[currentPage] | orderBy:sort.sortingOrder:sort.reverse  | filter:SearchText">
							<td>{{reviewedByOtherRev.reviewedDate}}</td>
							<td>{{reviewedByOtherRev.participantId}}</td>
							<td>{{reviewedByOtherRev.projTitle}}</td>
							<td>{{reviewedByOtherRev.projCategory.join(",")}}</td>
							<td><p ng-repeat="assignedByAndReviewDoneByMapper in reviewedByOtherRev.assignedByAndReviewDoneByMapperSet">{{assignedByAndReviewDoneByMapper.assignedByName}}</p></td>
							<td><p ng-repeat="assignedByAndReviewDoneByMapper in reviewedByOtherRev.assignedByAndReviewDoneByMapperSet">
										<a href="" data-toggle="modal"
											data-target="#ratingDetailsModal"
											ng-click="ShowDetailsReview(assignedByAndReviewDoneByMapper.ratingId,reviewedByOtherRev.projTitle,assignedByAndReviewDoneByMapper.reviewDoneByName)">{{assignedByAndReviewDoneByMapper.reviewDoneByName}}</a>
									</p></td>
							<td>{{reviewedByOtherRev.averageRating}}</td>
						</tr> 
					</table>
				</div>
			...
			<div class="col-xs-12"
				...
				<div class="carddemoBasicUsage">
					...
					<div flex-xs="" flex-gt-xs="33" layout="column"
						...
						Review</md-button> <md-button class="md-raised"
							ng-click="viewGYTIProjDetails(reviewedProjects.projId)"
							data-toggle="modal" data-target="#GYTIprojectDetailsModal">
						<i class="fa fa-eye" aria-hidden="true"></i>&nbsp;Details</md-button> </md-card-actions> </md-card>
					</div>
					</md-content>
				</div>
				</md-tab-body>
			...
			<div class="modal fade" data-easein="perspectiveDownIn" id="ratingDetailsModal" tabindex="-1"
