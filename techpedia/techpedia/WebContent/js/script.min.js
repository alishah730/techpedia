...
$(document).ready(function() {
	...
	$('.add-project-submit').click(function(e) {
		...
		// alert(challengeId);
		if (challengeId === 'null') {
			$.post('addProjectRequest', $('#addProjectInformation').serialize() + $('#addDetailedProjectInformationForm').serialize(), function(data) {
				if (data === 'Y') {
			...
			$.post('acceptChallengeRequest', $('#addProjectInformation').serialize() + $('#addDetailedProjectInformationForm').serialize() + "&challengId=" + challengeId, function(data) {
				if (data === 'Y') {
	...
	$('#add-challenge-button').click(function(e) {
		...
		errorString = '';
		captval = '1562';
		if ($("#captcha").val() != captval) {
			error = true;
			errorString = 'Captcha failed';
			random = Math.floor((Math.random() * 10) + 1);
			$captval = random;
			$('#captchaval').val(random);
		}
		if (error) {
			$('.error-place').show();
			...
			$.post('addChallengeRequest', $('#addChallengeForm').serialize(), function(data) {
				if (data.indexOf("success") >= 0) {
	...
	$('#login-submit').click(function(e) {
		...
		if (username.val() === '') {
			error = true;
			errorString += "Please Enter Username<br/>";
		}
		...
		if (error) {
			...
			$.post('ajax/login', $('#login-form').serialize(), function(data) {
				if (data === 'success') {
					...
					$('#login-error-div').show();
					$('#login-error-div').html("Invalid Username or Password");
				}

				$('#login-submit').html("Login");
				$('#login-submit').addClass("login");
			});
		}
	});
	...
	$('#add-new-faculty').click(function(e) {
		e.preventDefault();
	...
	$('#add-faculty-submit-button').click(function(e) {
		e.preventDefault();
		$.post('ajax/addNewFaculty', $('#add-new-faculty-form').serialize(), function(data) {
			if (data === 'N') {
				alert('Failed to add a faculty');
			} else {
				$('#projFaculty').append('<option selected="selected" value="' + data + '">' + $('#newFacultyName').val() + '</option>');
	...
	$('#search-team-members-submit-button').click(function(e) {
		e.preventDefault();
	...
	$('#choose-selected-team-members').click(function(e) {
		e.preventDefault();
	...
	$('.fouroverlaymentor').click(function(e) {
		e.preventDefault();
	...
	var setNumber = 0;
	$('.scrollDetectCollaborate').click(function(e) {
		e.preventDefault();
		$('#ajax-text').hide();
		$('#ajax-loader').show();
		setNumber += 1;
	...
	var setNumber = 0;
	$('.challengeScrollDetect').click(function(e) {
		e.preventDefault();
		$('#ajax-text').hide();
		$('#ajax-loader').show();
		setNumber += 1;
	...
	$('#login-button-new').click(function(e) {
		e.preventDefault();
	...
	$('#branchIdOfFaculty2').on("select2-blur", function(e) {
		$('#branchIdOfFaculty').val($('#branchIdOfFaculty2').val());
	});
	...
	$('#search-new-members-button').click(function(e) {
		e.preventDefault();
	...
	$('#search-new-members-submit-button').click(function(e) {
		e.preventDefault();
	...
	$('#choose-selected-new-members').click(function(e) {
		e.preventDefault();
